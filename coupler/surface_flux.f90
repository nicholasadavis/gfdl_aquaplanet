...
use             fms_mod, only: read_data, set_domain, NOTE, error_mesg
...
use      transforms_mod, only: get_sin_lat, get_grid_domain, grid_domain
use spectral_dynamics_mod, only: flux_average
...
integer :: id_eddy_u_stress, id_eddy_v_stress
...
! ---- local vars ----------------------------------------------------------
integer :: unit, ierr, io, is, ie, js, je
...
subroutine surface_flux_1d (                                           &
     ...
     dt,        land,      seawater,     avail, u_stress_eddy, v_stress_eddy, t_eddy, q_eddy)
!</PUBLICROUTINE>
...
! ============================================================================
  ...
  real, intent(in),  dimension(:) :: &
       ...
       gust, u_stress_eddy, v_stress_eddy, t_eddy, q_eddy
  ...
  real, dimension(size(t_atm(:))) ::                          &
       ...
       q_atm,    q_surf0,  dw_atmdu,  dw_atmdv, sin_lat
...
!     thv_surf= t_surf0                        ! surface virtual (potential) T -- just for testing tun off the q_surf
  ...
  if(alt_gustiness) then
     ...
     where(avail) 
        !add eddy contribution to wind speed
  ...
  where (avail)
     ...
     flux_t = rho_drag * (t_surf0 - th_atm) + t_eddy ! flux of sensible heat (W/m**2)
     ...
     ! evaporation
     rho_drag  =  drag_q * rho
     flux_q    =  rho_drag * (q_surf0 - q_atm) + q_eddy  ! flux of water vapor  (Kg/(m**2 s))
     ...
     ! stresses
     rho_drag   = drag_m * rho
     flux_u     = (rho_drag * u_dif) + u_stress_eddy  ! zonal      component of stress (Nt/m**2)
     flux_v     = (rho_drag * v_dif) + v_stress_eddy  ! meridional component of stress
...
subroutine surface_flux_0d (                                                 &
     ...
     dt,          land_0,       seawater_0,  avail_0, u_stress_eddy_0, v_stress_eddy_0, t_eddy_0, q_eddy_0)
  ...
  real, intent(in) ::                                                  &
       ...
       rough_mom_0, rough_heat_0, rough_moist_0, rough_scale_0, gust_0,&
       u_stress_eddy_0, v_stress_eddy_0, t_eddy_0, q_eddy_0
  real, intent(out) ::                                                 &
       flux_t_0,    flux_q_0,     flux_r_0,    flux_u_0,  flux_v_0,    &
       dhdt_surf_0, dedt_surf_0,  dedq_surf_0, drdt_surf_0,            &
       dhdt_atm_0,  dedq_atm_0,   dtaudu_atm_0,dtaudv_atm_0,           &
  ...
  real, dimension(1) :: &
       ...
       dhdt_atm,  dedq_atm,   dtaudu_atm,dtaudv_atm,         &
       w_atm,     u_star,     b_star,    q_star,             &
       cd_m,      cd_t,       cd_q, u_stress_eddy, v_stress_eddy, t_eddy, q_eddy
  ...
  avail(1)       = avail_0
  u_stress_eddy(1) = u_stress_eddy_0
  v_stress_eddy(1) = v_stress_eddy_0
  t_eddy(1)      = t_eddy_0
  q_eddy(1)      = q_eddy_0
...
subroutine surface_flux_2d (                                           &
  ...
  real, intent(in),  dimension(:,:) :: &
       ...
       p_surf,    t_surf,     u_surf,    v_surf,             &
       rough_mom, rough_heat, rough_moist, rough_scale, gust,&
       u_stress_eddy, v_stress_eddy, t_eddy, q_eddy
  real, intent(out), dimension(:,:) :: &
       flux_t,    flux_q,     flux_r,    flux_u,  flux_v,    &
       dhdt_surf, dedt_surf,  dedq_surf, drdt_surf,          &
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          ...
          dt,             land(:,j),       seawater(:,j),   avail(:,j),  u_stress_eddy(:,j), v_stress_eddy(:,j), t_eddy(:,j), q_eddy(:,j))
  end do
