...
use               fms_mod, only: mpp_pe, mpp_root_pe, error_mesg, FATAL, NOTE, write_version_number, set_domain, &
...
real, allocatable, dimension(:,:,:  ) :: dt_ug, dt_vg, dt_tg, dt_ug_damp, dt_vg_damp, dt_tg_damp, dt_ug_diff
...
allocate (dt_ug_diff   (is:ie, js:je, num_levels))
allocate (dt_ug_damp   (is:ie, js:je, num_levels))
allocate (dt_vg_damp   (is:ie, js:je, num_levels))
allocate (dt_tg_damp   (is:ie, js:je, num_levels))
...
call spectral_diagnostics(Time_next, psg(:,:,future), ug(:,:,:,future), vg(:,:,:,future), &
                          tg(:,:,:,future), wg_full, grid_tracers(:,:,:,future,:),dt_ug_damp, &
                          dt_vg_damp, dt_tg_damp, dt_ug_diff)
