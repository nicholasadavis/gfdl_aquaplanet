...
use              fms_mod, only: mpp_pe, mpp_root_pe, error_mesg, FATAL, NOTE, write_version_number
...
integer :: id_wf, id_z, id_drystaten, id_moiststaten, id_uv, id_vq, id_vdse, id_pt,    &
    ...
    id_entrop_tempcor, id_kegen, id_kegenq, id_kegenqtinv, id_ou, id_ov, id_ot, id_oq, &
    id_vv, id_vt, id_uu
...
id_uu  = register_diag_field(mod_name, 'uu', axes_3d, Time, 'uu', '(m/sec)**2')
id_vv  = register_diag_field(mod_name, 'vv', axes_3d, Time, 'vv', '(m/sec)**2')
id_vt  = register_diag_field(mod_name, 'vt', axes_3d, Time, 'vt', '(m/sec)**2')
id_ot  = register_diag_field(mod_name, 'ot', axes_3d, Time, 'ot', '(m/sec)**2')
id_ou  = register_diag_field(mod_name, 'ou', axes_3d, Time, 'ou', '(m/sec)**2')
id_ov  = register_diag_field(mod_name, 'ov', axes_3d, Time, 'ov', '(m/sec)**2')
id_oq  = register_diag_field(mod_name, 'oq', axes_3d, Time, 'oq', '(m/sec)**2')
...
if(id_uu > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = u_grid(:,:,k)*u_grid(:,:,k)
    enddo
    used = send_data(id_uu, tempdiag_3d, Time)
endif
if(id_vv > 0) then
do k=1,num_levels
tempdiag_3d(:,:,k) = v_grid(:,:,k)*v_grid(:,:,k)
enddo
used = send_data(id_vv, tempdiag_3d, Time)
endif

if(id_uv > 0) then
  do k=1,num_levels
     tempdiag_3d(:,:,k) = u_grid(:,:,k)*v_grid(:,:,k)
  enddo
  used = send_data(id_uv, tempdiag_3d, Time)
endif
if(id_vt > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = v_grid(:,:,k)*t_grid(:,:,k)
    enddo
    used = send_data(id_vt, tempdiag_3d, Time)
endif
if(id_vv > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = v_grid(:,:,k)*v_grid(:,:,k)
...
if(id_vq > 0) then
  do k=1,num_levels
    tempdiag_3d(:,:,k) = v_grid(:,:,k)*q_grid(:,:,k)
  enddo
  used = send_data(id_vq, tempdiag_3d, Time)
endif
if(id_ot > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = wg_full(:,:,k)*t_grid(:,:,k)
    enddo
used = send_data(id_uv, tempdiag_3d, Time)
endif
if(id_ou > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = wg_full(:,:,k)*u_grid(:,:,k)
    enddo
used = send_data(id_vt, tempdiag_3d, Time)
endif
if(id_ov > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = wg_full(:,:,k)*v_grid(:,:,k)
    enddo
used = send_data(id_vv, tempdiag_3d, Time)
endif
if(id_oq > 0) then
    do k=1,num_levels
        tempdiag_3d(:,:,k) = wg_full(:,:,k)*q_grid(:,:,k)
    enddo
used = send_data(id_vq, tempdiag_3d, Time)
endif
